...
class AlpacaTradeFeed:
    ...
    def __init__(self, symbols, on_trade, feed: str = "iex",
        ...
        self.feed = feed
        self.relay_url = relay_url    # if set, connect here instead of
                                      # Alpaca directly — see
                                      # alpaca_relay.py. Auth is still
                                      # sent (the relay ignores its
                                      # contents) so no other logic
    ...
    def run(self):
        ...
        url = self.relay_url or f"wss://stream.data.alpaca.markets/v2/{self.feed}"
        backoff = 1
